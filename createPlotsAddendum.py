...
colorArray1 = ("limegreen","seagreen","Red","darkblue","cornflowerblue")
...
def plotBar(ax, totalPerHundredThous, rows, fields, count, covidCount):
    ...
    medianCovidDeathNum = np.median(covidCount);
    if medianCovidDeathNum > 0:
        err=[[100000*(medianCovidDeathNum-min(covidCount))],[100000*(max(covidCount)-medianCovidDeathNum)]]
        ax.bar(count, medianCovidDeathNum*100000, bottom=deathSum, color="red", yerr=err);  
...
IFR_Array = (0.002, 0.002, 0.01, 0.01, 0.004);
aArray = (0.25, 0.5, 0.25, 0.5, 0.35);
...
for IFR in IFR_Array:
        ...
        labelArray.append(['S-IFR = ' + str(IFR*100) + '% ; Asymp = ' + str(a*100) + '% (Scenario ' + str(ii+1) + ')' ]);
        ii = ii + 1;
...
################
# sanity check on symptomatic case fatality rate by age
pAgeCondensed = np.zeros(3);
pAgeGivenDeathCondensed = np.zeros(3);
pAgeCondensed[0] = sum(pAge[0:4]);
pAgeGivenDeathCondensed[0] = sum(pAgeGivenDeath[0:4]);
pAgeCondensed[1] = pAge[4];
pAgeGivenDeathCondensed[1] = pAgeGivenDeath[4];    
pAgeCondensed[2] = sum(pAge[5:8]);
pAgeGivenDeathCondensed[2] = sum(pAgeGivenDeath[5:8]); 

pDeathGivenAge = (pAgeGivenDeathCondensed*0.01)/pAgeCondensed;